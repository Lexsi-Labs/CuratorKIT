...
| `hallucination_prompt_template` | `str \| None` | `None` | Custom hallucination judge prompt. Required variables: `{source_text}`, `{question}`, `{answer}`. Expected output JSON **must include `"grounding_score": 0.XX`** — that exact key is what the gate parses to decide pass/fail; see the note below. |
